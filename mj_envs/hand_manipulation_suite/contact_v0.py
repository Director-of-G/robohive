...
class ContactEnvV0(mujoco3_env.Mujoco3Env, utils.EzPickle):
    def __init__(self):
        ...
        mujoco3_env.Mujoco3Env.__init__(self, curr_dir+'/assets/DAPG_contact.xml', 5)
        
        # change actuator sensitivity
        # self.sim["model"].actuator_gainprm[get_actuator_id(self.sim, "A_WRJ1"):get_actuator_id(self.sim, "A_WRJ0")+1,:3] = np.array([50, 0, 0])
        # self.sim["model"].actuator_gainprm[get_actuator_id(self.sim, "A_FFJ3"):get_actuator_id(self.sim, 'A_THJ0')+1,:3] = np.array([10, 0, 0])
        # self.sim["model"].actuator_biasprm[get_actuator_id(self.sim, 'A_WRJ1'):get_actuator_id(self.sim, 'A_WRJ0')+1,:3] = np.array([0, -20, 0])
        # self.sim["model"].actuator_biasprm[get_actuator_id(self.sim, 'A_FFJ3'):get_actuator_id(self.sim, 'A_THJ0')+1,:3] = np.array([0, -10, 0])
        ...
        self.action_space.high = self.model.actuator_ctrlrange[:,1]
        self.action_space.low  = self.model.actuator_ctrlrange[:,0]
    ...
    def get_env_state(self):
        ...
        hand_qpos = qp[:30]
        palm_pos = self.data.site_xpos[self.S_grasp_sid].ravel()
        return dict(hand_qpos=hand_qpos, palm_pos=palm_pos,
            qpos=qp, qvel=qv)
